...
const AddRoom = () => {
    ...
    const { axios, getToken, selectedHotel } = useAppContext()
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            // Adding selected hotel ID
            formData.append('hotelId', selectedHotel._id)
    ...
    return (
        <form onSubmit={onSubmitHandler} className='pb-32'>
            ...
            <div className='flex flex-col lg:flex-row gap-8 items-start mt-6 mb-16'>
                <div className='flex-1'>
                    <p className='text-gray-800'>Amenities</p>
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6 mt-2 text-gray-400 max-w-2xl'>
                        {Object.keys(inputs.amenities).map((amenity, index) => (
                            <div key={index} className='flex items-center gap-2'>
                                ...
                                />
                                <label htmlFor={`amenities${index + 1}`} className='cursor-pointer select-none text-sm whitespace-nowrap'>{amenity}</label>
                            </div>
                        ))}
                    </div>
                </div>
                
                <button className='bg-primary text-white px-8 py-2 rounded cursor-pointer lg:mt-8' disabled={loading}>
