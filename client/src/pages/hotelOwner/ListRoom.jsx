...
const ListRoom = () => {
    ...
    const { axios, getToken, user, selectedHotel } = useAppContext()
    ...
    const fetchRooms = async () => {
        ...
        try {
            const { data } = await axios.get(`/api/rooms/owner?hotelId=${selectedHotel._id}`, { headers: { Authorization: `Bearer ${await getToken()}` } })
            if (data.success) {
                setRooms(data.rooms)
            }
            else {
    ...
    // Fetch Rooms when user is logged in or hotel is changed
    useEffect(() => {
