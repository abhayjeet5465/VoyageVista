...
const HotelReg = () => {
    const { setShowHotelReg, axios, getToken, setIsOwner, setOwnerHotels, setSelectedHotel } = useAppContext();
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            if (data.success) {
                console.log('Hotel registered successfully:', data.hotel);
                toast.success(data.message);
                setIsOwner(true);
                
                // Refresh hotel list in context
                const hotelsResponse = await axios.get('/api/hotels/owner', { 
                    headers: { Authorization: `Bearer ${await getToken()}` } 
                });
                if (hotelsResponse.data.success) {
                    console.log('Fetched hotels after registration:', hotelsResponse.data.hotels);
                    setOwnerHotels(hotelsResponse.data.hotels);
                    // Auto-select the newly created hotel
                    setSelectedHotel(data.hotel);
                    localStorage.setItem('selectedHotelId', data.hotel._id);
                    console.log('Selected hotel set to:', data.hotel);
                }
