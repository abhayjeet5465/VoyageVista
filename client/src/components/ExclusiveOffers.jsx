...
const ExclusiveOffers = () => {
    const handleViewAllOffers = () => {
        // TODO: Create dedicated offers page
        alert('View all offers feature coming soon!')
    }
    
    const handleViewOffer = (offer) => {
        // TODO: Create offer details page
        alert(`${offer.title} details coming soon!`)
    }
    ...
    return (
        <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pt-20 pb-30'>
            <div className='flex flex-col md:flex-row items-center justify-between w-full '>
                ...
                <button onClick={handleViewAllOffers} className='group flex items-center gap-2 font-medium cursor-pointer max-md:mt-12'>
                    View All Offers
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
                {exclusiveOffers.map((item) => (
                    <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{ backgroundImage: `url(${item.image})` }}>
                        ...
                        <button onClick={() => handleViewOffer(item)} className='flex items-center gap-2 font-medium cursor-pointer mt-4 mb-5'>
                            View Offers
